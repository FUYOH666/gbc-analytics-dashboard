...
Публичный деплой на Vercel не обязателен для работы с кодом, но удобен для демонстрации.

- **Локально:** раздел «Локальный запуск» ниже и файл `.env.local`.
- **Облако:** раздел «Деплой» — подключить репозиторий [FUYOH666/retail-crm-analytics-demo](https://github.com/FUYOH666/retail-crm-analytics-demo) к Vercel, задать переменные окружения, затем при желании добавить сюда стабильный публичный URL.
...
## Vercel, remote и данные

1. **Репозиторий:** [FUYOH666/retail-crm-analytics-demo](https://github.com/FUYOH666/retail-crm-analytics-demo). Для клона и `git remote`: `git@github.com:FUYOH666/retail-crm-analytics-demo.git` (или HTTPS).
2. **Vercel:** создайте проект из этого репозитория, перенесите переменные из `.env.example`, при необходимости обновите ссылку в [Live demo](#live-demo).
3. **Supabase:** старые строки в таблицах не очищаются при обновлении репо; при необходимости сбросьте тестовые данные или заведите отдельный проект.

Старые содержимые файлов могут по-прежнему встречаться в **истории Git** (прошлые коммиты); убрать их из истории можно только отдельными тяжёлыми шагами (`git filter-repo` и переписывание `main`).
